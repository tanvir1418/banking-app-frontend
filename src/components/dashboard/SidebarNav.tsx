    ...
    <div className="w-64 bg-sidebar text-sidebar-foreground flex flex-col min-h-screen border-r border-sidebar-border">
      <div className="p-6 border-b border-sidebar-border">
        ...
        {location.pathname !== '/dashboard' && location.pathname !== '/dashboard/' ? (
          <Link to="/dashboard">
            <Button variant="ghost" className="text-muted-foreground hover:text-primary mb-4 flex items-center text-sm px-0">
              <ChevronLeft className="mr-2 h-4 w-4" /> back to dashboard
            </Button>
          </Link>
        ) : (
          <Link to="/dashboard" className="flex items-center space-x-2 mb-4 h-[40px]"> {/* Ensure consistent height */}
            <img src="/placeholder.svg" alt="Banking App" className="h-8 w-auto" /> {/* Replace with actual logo */}
      ...
      <nav className="flex-grow px-4 py-4 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`h-5 w-5 ${isActive ? 'text-sidebar-primary-foreground' : 'text-muted-foreground group-hover:text-sidebar-accent-foreground'}`} />
              <span>{item.name}</span>
            </Link>
          );
        })}
